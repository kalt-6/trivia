...
window.App = {
    ...
    init: async () => {
        ...
        try {
            ...
            // NEW: Check URL for a specific quiz to load directly
            ...
            if (quizTitleFromURL) {
                ...
                if (targetQuiz) {
                    // false = don't push state to history again since we just landed here
    ...
    // --- NEW: Helper to safely return to home screen & clear URL ---
    goHome: () => {
        // Clears the ?quiz=... from the URL bar
    ...
    renderHome: () => {
        ...
        State.categories.forEach(cat => {
            ...
            const active = isActive ? "bg-primary text-white border-primary shadow-lg transform scale-105" : "bg-surface text-text-light border-gray-200 hover:border-primary hover:text-primary";
    ...
    // --- Game Logic ---
    // NEW: Added pushState parameter so URL updating works cleanly
    startQuiz: async (quizId, title, pushState = true) => {
        if (pushState) {
            const quizTitleURL = encodeURIComponent(title);
            window.history.pushState({}, '', `?quiz=${quizTitleURL}`);
        }
        window.scrollTo({ top: 0, behavior: 'smooth' });
        ...
        try {
            ...
            if (!data || data.length === 0) {
                ...
                <div class="text-center text-red-500 mt-20 bouncy-card p-10 max-w-md mx-auto fade-in">
                    ...
                    <button onclick="App.goHome()" class="mt-6 btn-3d bg-primary text-white py-3 px-6 rounded-xl font-bold uppercase">Back to Vault</button>
                </div>`;
                return;
            }
...
// --- NEW: Handle Browser "Back" and "Forward" buttons perfectly ---
window.addEventListener('popstate', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const quizTitleFromURL = urlParams.get('quiz');
    
    // If the user hit back to a URL that has a quiz title in it
    ...
    // If the user hit back to the home screen (no quiz in URL)
    if (State.quizzes.length > 0) {
        App.renderHome();
    } else {
        App.init(); // Failsafe if state was somehow cleared
    }
});
